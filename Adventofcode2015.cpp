...
void D_1_2(string Input){
    ...
    for(int i=0;i!=10000;i++){
     ...
     cout << "First time in the basement is at Position " << pos << endl;
}

void D_2_1(string Input){
    char sepperator = '-'; // Change here if need be
    int lwh = 1 ,temp,l,w,h,tarea=0,area=0; //Temp is reused
    for(int i=0,temp;i!=100000;i++){

    if(Input[i] == 'x'){
        lwh++;
        continue;}

    if(Input[i] == sepperator){
        //cout << "Found package with the dimensions: " << l <<"x"<< w <<"x"<< h<< " \n";
        tarea = 2*l*w;
        temp = l*w;
        ...
        tarea += 2*w*h;
        tarea += temp; //Extra to wrap
        //cout << "Adding" << tarea << "square feet \n";
        area += tarea;
        lwh = 1;
        continue;}
    ...
    if(Input[i] >= 48 && Input[i] <= 57){
        temp = 0;
        if(Input[i+1] >= 48 && Input[i+1] <= 57){ //if True number is not 0-9 but 10-99
            temp = 10*(Input[i]-48);
            i++;
        }
        temp += (Input[i]-48);
    ...
    cout << area <<" square feet needed";
}

void D_2_2(string Input){
    char sepperator = '-'; // Change here if need be
    int lwh = 1 ,temp,l,w,h,volume=0,tribbon,ribbon=0; //Temp is reused
    for(int i=0,temp;i<=400000;i++){
    ...
    if(Input[i] == sepperator){
        ...
        tribbon = volume + 2*temp;
        //cout << "With a Volume of " << volume <<" and Wrap of  2*"<< temp << " \n";
        ribbon += tribbon;
        lwh = 1;
        continue;}
    ...
    cout << ribbon <<" feet of ribbon needed";
}

void D_3_1(string Input){
    ...
    cout << "Visited " << visited <<" Houses. \n";
}

void D_3_2(string Input){

    int Field [1000][1000] = {0};
    int pos_x = 500,pos_y = 500;
    int r_pos_x = 500,r_pos_y = 500;
    bool flesh = true;
    Field [pos_x][pos_y] = 1;
    int visited = 1;

    for(int i=0,temp;i<=40000;i++){
        if(flesh){
            if(Input[i] == '^')
                pos_y++;
            else if(Input[i] == 'v')
            ...
            if(Field [r_pos_x][r_pos_y] != 1){
                Field [r_pos_x][r_pos_y] = 1;
                visited++;
        }
        cout << "Robot now at" << r_pos_x <<"-"<< r_pos_y<< "\n";
        flesh = true;
        }
    }
    cout << "Visited " << visited <<" Houses. \n";
